...
//					}
					...
					if (filename != null && !filename.equals("")) {
						...
						// 在服务器创建同名文件
						String webPath = "E:\\code\\JavaWeb\\JavaWeb\\NoteBoard\\web\\upload\\";
						//将服务器中文件夹路径与文件名组合成完整的服务器端路径
						String filepath = webPath + filename;
						SqlSession sqlSession = MySessionUtils.getSession();
						UserMapper userMapper = sqlSession.getMapper(UserMapper.class);
						UpdateAvatarDTO updateAvatarDTO = UpdateAvatarDTO.builder()
								.id((Integer) request.getSession().getAttribute("id"))
								.avatar("/NoteBoard/upload/" + filename)
								.build();
						userMapper.updateAvatar(updateAvatarDTO);
						sqlSession.commit();
						...
						response.sendRedirect("/NoteBoard/view/message.jsp");
					}
				}
			}
		} catch (Exception e) {
